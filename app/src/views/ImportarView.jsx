...
// Tipos de importação que têm a própria tela (fora dos botões desta).
const TITULOS_EXTRAS = { prospectos: "Lista de prospecção" };
      ...
      {historico.length > 0 && (
        <div className="importar-historico">
          ...
          <ul>
            {historico.map((h) => (
              <li key={h.id}>
                <b>{ORIGENS[h.tipo]?.titulo || TITULOS_EXTRAS[h.tipo] || h.tipo}</b>
